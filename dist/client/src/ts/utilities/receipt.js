...
function calculateSummary(subtotal, options, includeFeesInTaxBase) {
...
export function calculateReceiptSummary(subtotal, options) {
    return calculateSummary(subtotal, options, false);
}
export function calculateSalesReportSummary(subtotal, options) {
...
export function normalizeVendorId(value) {
...
export function getDisplayItemName(item) {
...
export function getLineBasePrice(item) {
...
export function openItemizedReceipt(cashPayment, items, ticketLabel) {
    if (items.length === 0) {
        alert('No items to generate a receipt.');
        return;
    }
    const receiptItems = items.filter(item => !item.refunded).map(item => {
