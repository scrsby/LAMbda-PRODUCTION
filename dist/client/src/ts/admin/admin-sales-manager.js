...
async function generateReport() {
    try {
        ...
        const salesResponse = await apiAxios(salesEndpoint, { method: 'GET' });
        ...
        const ticketDetails = await Promise.all(tickets.map(async (ticket) => {
            return await apiAxios(`/POS/ticket/${ticket.ticket_id}`, { method: 'GET' });
        }));
        const reportItems = [];
...
function renderDailyStats(salesTotal, commission) {
...
function renderSalesMessage(message) {
    if (!salesTableBody)
        return;
    destroySalesTablePagination();
    salesTableBody.innerHTML = `
...
//# sourceMappingURL=admin-sales-manager.js.map