...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('vendor-search-btn')?.addEventListener('click', () => {
        const query = document.getElementById('vendor-id-field').value.trim().toLowerCase();
 ...
 */
async function createVendor(vendorId) {
    try {
        const body = {};
        if (vendorId !== null)
...
function openDeleteModal(vendorId) {
    ...
    if (message)
        message.textContent = `This will permanently delete Vendor #${vendorId}. This action cannot be undone.`;
    if (modal)
...
//# sourceMappingURL=admin-vendor-settings.js.map