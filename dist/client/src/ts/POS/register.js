...
import { openItemizedReceipt, escapeHtml, calculateReceiptSummary, calculateSalesReportSummary, getDisplayItemName, getLineBasePrice, getLineFinalPrice } from '../utilities/receipt.js';
let validVendorIds = new Set();
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (vendorResponse?.vendorIds) {
            const normalizedVendorIds = vendorResponse.vendorIds
                .map((id) => Number(id))
...
let searchResultsDataTable = null;
let itemSearchDebounceId;
...
const searchForm = document.getElementById('input_box');
...
const searchAndAddSection = document.getElementById('search-and-add');
const posTopSection = document.querySelector('.pos-top-section');
const taxExemptCheckbox = document.getElementById('tax-exempt-checkbox');
const taxExemptForm = document.getElementById('tax-exempt-form');
const taxExemptFormLabel = document.getElementById('tax-exempt-form-label');
const cashPaymentCheckbox = document.getElementById('cash-payment-checkbox');
...
function setTaxExemptUi(isChecked, businessName = '') {
...
function setPaymentAndTaxControlsEnabled(enabled) {
...
function setClosedTicketState(ticketId) {
    ticketActive = false;
    ticketReadOnly = true;
    ticketDirty = false;
    currentTicketId = ticketId;
    ...
    searchAndAddSection.style = '';
    if (posTopSection)
        posTopSection.style.cssText = 'opacity: 0.6; pointer-events: none;';
    primaryBtn.textContent = 'Update Ticket';
...
createItemBtn?.addEventListener('click', () => {
    if (!vendorIdInput || !itemNameInput || !vendorPriceInput) {
        ...
        createItemLocally(vendor_id, vendor_inventory_id, formatItemDescription(baseName, quantity), roundCurrency(tagPrice * quantity));
        vendorIdInput.value = '';
        if (vendorInventoryIdInput)
...
cashPaymentCheckbox?.addEventListener('change', async function () {
    ...
    if (!ticketReadOnly || !isAdminUser || !currentTicketId)
        return;
...
async function updateTicket() {
    ...
    try {
        ...
        const message = error?.response?.data?.error ?? 'Failed to update ticket. Please try again.';
...
function bindDiscountCalculationInputs(row) {
    const subtotalInput = row.querySelector('[data-field="subtotal"]');
    const discountPercentInput = row.querySelector('[data-field="discount_percent"]');
    const discountAmountInput = row.querySelector('[data-field="discount_amount"]');
    const finalPriceInput = row.querySelector('[data-field="final_price"]');
    if (!subtotalInput || !discountPercentInput || !discountAmountInput || !finalPriceInput)
        return;
    ...
    const recalcFromFinalPrice = () => {
        ...
        if (isNaN(subtotal) || isNaN(finalPrice))
            return;
...
function toggleItemEditMode(index, row) {
...
function saveEditedItem(index, row) {
    ...
    const vendorIdInput = row.querySelector('[data-field="vendor_id"]');
    const vendorInventoryIdInput = row.querySelector('[data-field="vendor_inventory_id"]');
...
function getCombinedItem(index) {
...
function setCombinedItem(index, item) {
    if (index < ticket_items.length) {
...
function createItemLocally(vendor_id, vendor_inventory_id, name, vendor_price) {
    const newItem = { vendor_id, vendor_inventory_id, name, quantity: 1, vendor_price, discount_percent: 0, discount_amount: 0, final_price: roundCurrency(vendor_price) };
...
function renderSearchResultsMessage(message) {
...
function renderSearchResults(items) {
    ...
    items.forEach((item) => {
        ...
        row.querySelector('button')?.addEventListener('click', () => {
            ...
            createItemLocally(item.vendorId, inventoryCode, formatItemDescription(item.itemName, quantity), roundCurrency(Number(item.price) * quantity));
...
async function searchInventory(manualSearch) {
    const itemName = itemNameInput?.value.trim() ?? '';
...
const checkoutFinalCharge = document.getElementById('checkout-modal-final-charge');
const checkoutCashPaymentCheckbox = document.getElementById('checkout-cash-payment-checkbox');
const checkoutGoBackBtn = document.getElementById('checkout-go-back-btn');
...
document.querySelectorAll('a.nav-link, .mobile-menu-nav a').forEach((link) => {
    link.addEventListener('click', (e) => {
        if (ticketActive && ticketDirty) {
            const confirmed = window.confirm('You have unsaved items on this ticket. Do you want to save before leaving?');
            if (confirmed) {
                e.preventDefault();
