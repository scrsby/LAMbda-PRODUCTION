...
async function getTicketDetail(ticketId) {
    const ticketResult = await db.query(`SELECT t.ticket_id,
                t.cashier_id,
                t.created_at,
                t.ticket_status,
    ...
    const itemsResult = await db.query(`SELECT ticket_item_id,
                ticket_id,
                vendor_id,
...
async function getTicketSummaries(filters) {
    ...
    const result = await db.query(`SELECT t.ticket_id,
                t.cashier_id,
                t.created_at,
                t.ticket_status,
                COALESCE(t.total, 0)::float AS total,
         ...
         LEFT JOIN users u ON u.user_id = t.cashier_id
         ${whereClause}
         ORDER BY t.created_at DESC`, values);
    return result.rows;
}
...
router.post('/create-ticket', posWriteRateLimit, async (req, res) => {
    try {
        ...
        const newTicket = await db.query('INSERT INTO tickets (ticket_status, created_at, cashier_id) VALUES ($1, NOW(), $2) RETURNING ticket_id, created_at', ['open', cashierId]);
...
router.post('/update-ticket', posWriteRateLimit, async (req, res) => {
    ...
    try {
        ...
        const ticketResult = await client.query('SELECT ticket_status FROM tickets WHERE ticket_id = $1 FOR UPDATE', [ticketId]);
        if (ticketResult.rows.length === 0) {
        ...
        const currentResult = await client.query('SELECT ticket_item_id FROM ticket_items WHERE ticket_id = $1', [ticketId]);
        const dbIds = currentResult.rows.map((r) => r.ticket_item_id);
        const frontendIds = ticketItems
            .filter((i) => i.ticket_item_id != null)
            .map((i) => i.ticket_item_id);
        ...
        for (const item of ticketItems) {
            ...
            await client.query(`UPDATE ticket_items
                 ...
                 WHERE ticket_item_id = $11 AND ticket_id = $12`, [item.vendor_id, item.vendor_inventory_id, item.name, item.vendor_price,
        ...
        for (const item of unsyncedItems) {
            const commission = parseFloat((item.final_price * 0.10).toFixed(2));
            const payout = parseFloat((item.final_price * 0.90).toFixed(2));
            const result = await client.query(`INSERT INTO ticket_items
                 ...
                 RETURNING ticket_item_id`, [ticketId, item.vendor_id, item.vendor_inventory_id, item.name,
        ...
        const totalResult = await client.query('SELECT COALESCE(SUM(final_price), 0) AS total FROM ticket_items WHERE ticket_id = $1', [ticketId]);
        const total = parseFloat(totalResult.rows[0].total);
        await client.query('UPDATE tickets SET total = $1, cash_payment = $2, tax_exempt = $3 WHERE ticket_id = $4', [total, cashPayment, taxExempt, ticketId]);
        await client.query('DELETE FROM tax_exempt_details WHERE ticket_id = $1', [ticketId]);
        if (taxExempt) {
            await client.query('INSERT INTO tax_exempt_details (ticket_id, business_name) VALUES ($1, $2)', [ticketId, taxExemptBusinessName]);
        }
...
router.post('/close-ticket', posWriteRateLimit, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        transactionStarted = true;
        const result = await client.query(`UPDATE tickets
            SET ticket_status = 'closed',
                cash_payment = $1,
                tax_exempt = $2
            WHERE ticket_id = $3 AND ticket_status = 'open'
            RETURNING ticket_id`, [cashPayment, taxExempt, ticketId]);
...
router.patch('/ticket/:id/payment-type', requireUserType('admin'), posWriteRateLimit, async (req, res) => {
    ...
    const cashPayment = req.body?.cashPayment === true;
    try {
        const result = await db.query(`UPDATE tickets SET cash_payment = $1 WHERE ticket_id = $2 RETURNING ticket_id, cash_payment`, [cashPayment, ticketId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Ticket not found' });
        }
...
router.patch('/ticket/:id/status', requireUserType('admin'), posWriteRateLimit, async (req, res) => {
    ...
    try {
        const result = await db.query(`UPDATE tickets SET ticket_status = $1 WHERE ticket_id = $2 RETURNING ticket_id, ticket_status`, [status, ticketId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Ticket not found' });
        }
...
router.patch('/ticket-item/:id/refund', requireUserType('admin'), posWriteRateLimit, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const itemResult = await client.query(`UPDATE ticket_items SET refunded = true
             WHERE ticket_item_id = $1 AND COALESCE(refunded, false) = false
             RETURNING ticket_id`, [itemId]);
        if (itemResult.rows.length === 0) {
            await client.query('ROLLBACK');
            // Distinguish "not found" from "already refunded"
            const exists = await client.query('SELECT ticket_item_id FROM ticket_items WHERE ticket_item_id = $1', [itemId]);
        ...
        const totalResult = await client.query(`SELECT COALESCE(SUM(final_price), 0)::float AS total
             FROM ticket_items
             WHERE ticket_id = $1 AND COALESCE(refunded, false) = false`, [ticketId]);
...
router.get('/vendors', async (_req, res) => {
    try {
        ...
        res.status(200).json({ vendorIds: result.rows.map((r) => r.vendor_id) });
    }
    catch (error) {
