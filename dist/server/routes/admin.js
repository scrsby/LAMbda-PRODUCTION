...
function createMagicLink(email, access_token, baseUrl) {
    const magicLink = `${baseUrl}/auth/create-account?token=${access_token}&email=${encodeURIComponent(email)}`;
    return magicLink;
}
async function sendAccessTokenEmail(email, accessToken, baseUrl) {
    const mailOptions = {
...
router.get('/getAllAccessTokens', async (req, res) => {
    try {
        const query = `
            SELECT
                email,
...
router.post('/createNewUser', requireAuth, requireUserType('admin'), adminRouteRateLimit, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const existingToken = await client.query('SELECT expires_at FROM access_tokens WHERE email = $1', [email]);
...
router.post('/regenerateAccessToken', requireAuth, requireUserType('admin'), adminRouteRateLimit, async (req, res) => {
 ...
 */
router.post('/createVendor', requireAuth, requireUserType('admin'), adminRouteRateLimit, async (req, res) => {
    ...
    try {
        ...
        if (vendor_id !== undefined && vendor_id !== null && vendor_id !== '') {
            ...
            result = await db.query('INSERT INTO vendors (vendor_id) VALUES ($1) RETURNING vendor_id, created_at', [id]);
        }
        else {
            result = await db.query('INSERT INTO vendors DEFAULT VALUES RETURNING vendor_id, created_at');
        }
 ...
 */
router.delete('/deleteVendor/:vendorId', requireAuth, requireUserType('admin'), adminRouteRateLimit, async (req, res) => {
...
router.get('/sales', requireAuth, requireUserType('admin'), adminRouteRateLimit, async (req, res) => {
    ...
    const conditions = [`t.ticket_status IN ('closed', 'partially_refunded', 'refunded')`];
    const values = [];
    if (orderId) {
        values.push(`%${orderId}%`);
    ...
    try {
        const ticketsResult = await db.query(`SELECT t.ticket_id,
                    t.cashier_id,
                    t.created_at,
                    t.ticket_status,
        ...
        const dailyResult = await db.query(`SELECT COALESCE(SUM(total), 0)::float AS daily_total
             FROM tickets
             WHERE ticket_status IN ('closed', 'partially_refunded', 'refunded')
               AND created_at >= $1
               AND created_at <= $2`, [startOfDay, endOfDay]);
        const dailySalesTotal = dailyResult.rows[0].daily_total;
