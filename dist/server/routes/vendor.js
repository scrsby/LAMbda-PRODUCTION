 ...
 */
router.get('/sales', requireAuth, requireUserType('vendor'), vendorRouteRateLimit, async (req, res) => {
    ...
    try {
        ...
        const dailyResult = await db.query(`SELECT COALESCE(SUM(ti.final_price), 0)::float AS daily_total
             FROM ticket_items ti
             JOIN tickets t ON t.ticket_id = ti.ticket_id
             WHERE ti.vendor_id = $1
               ...
               AND t.created_at >= $2
               AND t.created_at <= $3`, [vendorId, startOfDay, endOfDay]);
