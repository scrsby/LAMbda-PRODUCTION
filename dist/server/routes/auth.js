...
function getAuthFailureMessage(error, defaultMessage) {
...
function createMagicLink(email, resetToken, baseUrl) {
    const magicLink = `${baseUrl}/auth/reset-password?token=${resetToken}&email=${encodeURIComponent(email)}`;
    return magicLink;
}
async function sendResetPasswordEmail(email, resetToken, baseUrl) {
    const mailOptions = {
...
router.post('/generateResetPasswordToken', forgotPasswordRateLimit, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await client.query('INSERT INTO reset_password_tokens (email, reset_password_token) VALUES ($1, $2)', [email, resetPasswordToken]);
...
router.post('/resetPassword', resetPasswordRateLimit, async (req, res) => {
